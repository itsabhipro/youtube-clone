...
const VideoDetails = () => {
  ...
  useEffect(()=>{
    fetchData({endPoint:'videos',id:id}).then((data)=>{setVideo(data.items[0])});
    fetchData({relatedToVideoId:id,type:'video'}).then((data)=>setRelatedVideos(data.items));
  },[id]);
  if(!video?.snippet && !relatedVideos) return '....Loading';
  const {snippet:{title,channelId,channelTitle},statistics:{viewCount, likeCount}} = video;
  return (
    <Box minHeight='95vh'>
      <Stack direction={{md:'row',xs:'column'}}>
        <Box flex={1}>
          <Box sx={{width:'100%', position:'sticky', top:'86px',overflowY:'auto'}}>
